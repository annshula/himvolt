...
export function OrderRow({ order }: { order: OrderSummary }) {
  ...
  return (
    <Link
      ...
      className="group flex items-center justify-between gap-3 rounded-(--radius-card) border border-line bg-ivory px-4 py-4 shadow-sm transition-[border-color,box-shadow] duration-500 ease-[var(--ease-out-expo)] hover:border-ink/40 hover:shadow-[var(--shadow-lift)] sm:gap-4 sm:px-5"
    >
      <div className="flex min-w-0 items-center gap-3 sm:gap-4">
        ...
        <div className="min-w-0">
          <p className="font-display wrap-break-word text-[0.95rem] font-semibold text-ink transition-colors duration-300 group-hover:text-ink">
            {order.name}
          </p>
      ...
      <div className="flex shrink-0 items-center gap-2 sm:gap-3">
        ...
        <Icon
          name="arrow-right"
          className="size-4 text-ink-mute transition-transform duration-500 ease-(--ease-out-expo) group-hover:translate-x-1 group-hover:text-ink"
        />
      </div>
    </Link>
  );
